                                               ...
                                               float step_rate) 
{
    ...
    _params[param] = ParameterInfo{ addr, mi, ma, step_rate };
}
...
std::unordered_map<std::string, TunableParametersMap::ParameterInfo>::iterator 
TunableParametersMap::begin() {
    return _params.begin();
}

std::unordered_map<std::string, TunableParametersMap::ParameterInfo>::iterator 
TunableParametersMap::end() {
    return _params.end();
}

std::unordered_map<std::string, TunableParametersMap::ParameterInfo>::const_iterator 
TunableParametersMap::cbegin() const {
    return _params.cbegin();
}

std::unordered_map<std::string, TunableParametersMap::ParameterInfo>::const_iterator 
TunableParametersMap::cend() const {
    return _params.cend();
}
