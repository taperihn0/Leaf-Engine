...
UniversalChessInterface::Options UniversalChessInterface::_options = { 
		...
		OptionClearHash(),
		{ // --- Tunable parameters
		OptionTunableParam(SpinType<double>(MaxQuietsHistoryPow, 1.,   16.),    "MaxQuietsHistoryPow",  1.),
		OptionTunableParam(SpinType<double>(IidDepth,            1.,   10.),    "IidDepth", 		    1.3),
		OptionTunableParam(SpinType<double>(IidDepthDiv,         4.,   16.),    "IidDepthDiv", 		    1.3),
		OptionTunableParam(SpinType<double>(RfpDepth,     	     1.,   7.),     "RfpDepth", 		    1.3),
		...
		OptionTunableParam(SpinType<double>(NullReduction,       10.,  32.),    "NullReduction", 		1.1),
		...
		OptionTunableParam(SpinType<double>(FutilityMoveCount,   0.,   32.),    "FutilityMoveCount", 	0.8),
		OptionTunableParam(SpinType<double>(FutilityDelta,       2.,   216.),   "FutilityDelta", 		1.),
		OptionTunableParam(SpinType<double>(RazorBaseDelta,      20.,  500.),   "RazorBaseDelta", 		1.),
		OptionTunableParam(SpinType<double>(QMaterialDelta,      200., 1500.),  "QMaterialDelta", 		1.),
		OptionTunableParam(SpinType<double>(QProbeDepth,         -5.,  5.),     "QProbeDepth", 			1.2),
		//OptionTunableParam(SpinType<double>(ContemptFactor,      0.,   50.),    "ContemptFactor", 1.),
		OptionTunableParam(SpinType<double>(NNEvalScale,         2.,   12.),    "NNEvalScale", 			1.3),
		OptionTunableParam(SpinType<double>(ImprovingRate, 		 20,   100.),   "ImprovingRate" , 	  	1.3),
		OptionTunableParam(SpinType<double>(RfpImprovingSink, 	 1,    7.),     "RfpImprovingSink",   	1.2),
		OptionTunableParam(SpinType<double>(NullMargin, 		 10,   150.),   "NullMargin", 			1.1),
		OptionTunableParam(SpinType<double>(NullImprovingSink ,  1,    7.), 	"NullImprovingSink", 	1.2),
		OptionTunableParam(SpinType<double>(DynImprovementDepth, 3,    8.),     "DynImprovementDepth",  1.8),
		OptionTunableParam(SpinType<double>(TTEvalCorrRate, 	 2,    6.),     "TTEvalCorrRate", 		1.2),
		OptionTunableParam(SpinType<double>(NullDiffScale, 	 	 300., 1500.),  "NullDiffScale", 		0.7),
		}, 
};
